...
- Colors are normalized into coarse categories (PRIMARY COLORS ONLY):
  - red
  - blue
  - green
  - yellow
...
### Stage 1: Person Detection Module (YOLOv8 Detection)

```bash
cd "d:\FYP MODELS\PIPELINE\person_detection_module"
...
# Custom confidence threshold (default: 0.45)
python run_detection.py --conf 0.65

# Combine options
python run_detection.py --source "path/to/video.mp4" --conf 0.55
```

### Stage 2: Multi-Object Tracking Module (DeepSORT / ByteTrack)

```bash
cd "d:\FYP MODELS\PIPELINE\multi_object_tracking_module"
# Default: webcam
...
# Specify tracking backend (deepsort or bytetrack)
python run_tracking.py --backend "deepsort"
python run_tracking.py --backend "bytetrack"

# With detection threshold
python run_tracking.py --source "path/to/video.mp4" --det_thresh 0.45
```

### Stage 3A: Facial Recognition Module (Face Feature Extraction)

```bash
cd "d:\FYP MODELS\PIPELINE\facial_recognition_module"
# Train face embeddings on known persons
python entrypoint/train.py

# Run inference on video/image
python entrypoint/inference.py --source "path/to/video.mp4"

# Or use face node directly
python src/run_face_node.py
```

### Stage 3B: Clothing Feature Extraction Module

```bash
cd "d:\FYP MODELS\PIPELINE\clothing_feature_extraction_module"
# Run clothing color detection
python clothing_node.py --source "path/to/video.mp4"

# Color detection only (no tracking)
python colordetection.py --source "path/to/image.jpg"
```

### Stage 3C: Height Estimation Module (Pose + Height)

```bash
cd "d:\FYP MODELS\PIPELINE\height_estimation_module"
# Estimate height from pose
python estimator.py --source "path/to/video.mp4"

# Single image
python estimator.py --source "path/to/image.jpg"
```

### Stage 4: Multi-Attribute Fusion Module

```bash
cd "d:\FYP MODELS\PIPELINE\multi_attribute_fusion_module"
# Run fusion engine
python fuser.py

# With custom weights
python fuser.py --face_weight 0.70 --clothing_weight 0.30
```

### Stage 5: Temporal Validation Module

```bash
cd "d:\FYP MODELS\PIPELINE\temporal_validation_module"
# Run temporal validation
python validator.py

# With custom frame threshold
python validator.py --min_frames 5 --score_threshold 0.65
```

### Stage 6: Alert Decision Module

```bash
cd "d:\FYP MODELS\PIPELINE\alert_decision_module"
# Run alert decision engine
python decision.py

# Custom alert threshold and cooldown
python decision.py --threshold 0.75 --cooldown 30
```

### Stage 7: Explainability + Output Delivery

```bash
cd "d:\FYP MODELS\PIPELINE\explainability_module"
# Generate explanations
python engine.py

# And output delivery
cd "d:\FYP MODELS\PIPELINE\output_delivery_module"
python delivery.py
```

### Complete Backend Pipeline (All Stages Together)

```bash
cd "d:\FYP MODELS\PIPELINE"
# Run full pipeline with webcam
python run_surveillance_pipeline.py

# Full pipeline with video file
python run_surveillance_pipeline.py --source "path/to/video.mp4"

# Full pipeline with custom detection threshold
python run_surveillance_pipeline.py --conf 0.55

# Full pipeline with custom alert threshold
python run_surveillance_pipeline.py --alert_threshold 0.75

# Full pipeline with all custom parameters
python run_surveillance_pipeline.py --source 0 --conf 0.45 --alert_threshold 0.75 --cooldown 30
```

### Live Service + Web Dashboard

```bash
cd "d:\FYP MODELS\PIPELINE"
# Start backend pipeline + API server + dashboard
python surveillance_live_service.py

# With webcam (default)
python surveillance_live_service.py --source 0

# With video file
python surveillance_live_service.py --source "path/to/video.mp4"

# With custom port
python surveillance_live_service.py --port 5000
...
pytest facial_recognition_module/tests/
pytest alert_decision_module/tests/
pytest multi_attribute_fusion_module/tests/
pytest temporal_validation_module/tests/
...
### Module-by-Module Execution Order (Recommended for Testing)
...
# Terminal 4: Start live dashboard service
cd "d:\FYP MODELS\PIPELINE"
python surveillance_live_service.py
...
```bash
# One-command full system with webcam + dashboard
cd "d:\FYP MODELS\PIPELINE"
python surveillance_live_service.py --source 0
